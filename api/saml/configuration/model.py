        ...
        if not SAMLConfiguration.federated_identity_provider_entity_ids.options:
            try:
                ...
                incommon_federated_identity_providers = (
                    app._db.query(SAMLFederatedIdentityProvider)
                    .join(SAMLFederation)
                ...
                for (
                    ...
                    configuration_options.append(
                        ConfigurationOption(
                            key=incommon_federated_identity_provider.entity_id,
                            label=incommon_federated_identity_provider.display_name,
                        )
                    )
                    app._db.expunge(incommon_federated_identity_provider)
