...
class CoverageProviderTest(DatabaseTest):
    ...
    def bibliographic_data(self):
        return Metadata(
            ...
            published=datetime.datetime(1998, 3, 1, 0, 0, tzinfo=pytz.UTC),
...
class TestBaseCoverageProvider(CoverageProviderTest):
    ...
    def test_instantiation(self):
        ...
        now = cutoff_time=datetime.datetime.now(tz=pytz.UTC)
    ...
    def test_run_with_custom_result(self):
        ...
        start = datetime.datetime(2011, 1, 1, tzinfo=pytz.UTC)
        finish = datetime.datetime(2012, 1, 1, tzinfo=pytz.UTC)
        counter = -100
    ...
    def test_should_update(self):
        ...
        """
        cutoff = datetime.datetime(2016, 1, 1, tzinfo=pytz.UTC)
        ...
        record.timestamp = datetime.datetime(2015, 1, 1, tzinfo=pytz.UTC)
...
class TestIdentifierCoverageProvider(CoverageProviderTest):
    ...
    def test_run_on_specific_identifiers_respects_cutoff_time(self):
        ...
        last_run = datetime.datetime(2016, 1, 1, tzinfo=pytz.UTC)
        ...
        provider.cutoff_time = datetime.datetime(2016, 2, 1, tzinfo=pytz.UTC)
        (success, transient_failure, persistent_failure), records = (
            provider.run_on_specific_identifiers([self.identifier])
        )
        assert 0 == success
    ...
    def test_run_never_successful(self):
        ...
        assert (datetime.datetime.now(tz=pytz.UTC) - value).total_seconds() < 1
...
class TestCollectionCoverageProvider(CoverageProviderTest):
    ...
    def test_items_that_need_coverage(self):
        ...
        identifier = self._identifier()
        cutoff_time = datetime.datetime(2016, 1, 1, tzinfo=pytz.UTC)
        provider = AlwaysSuccessfulCoverageProvider(self._db)
...
class TestWorkCoverageProvider(DatabaseTest):
    ...
    def test_transient_failure(self):
        ...
        assert (datetime.datetime.now(tz=pytz.UTC)-value).total_seconds() < 2
