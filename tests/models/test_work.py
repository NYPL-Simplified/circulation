...
class TestWork(DatabaseTest):
    ...
    def test_to_search_document(self):
        ...
        for pool in work.license_pools:
            ...
            # The availability time was generated by a database
            # function, and we can't use strftime to get the same
            # level of sub-second precision, but we can check that
            # it's basically the same time.
            availability = pool.availability_time.strftime("%Y-%m-%dT%H:%M:%S.")
            assert match['availability_time'].startswith(availability)
