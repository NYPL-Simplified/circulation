...
class TestWork(DatabaseTest):
    ...
    def test_to_search_document(self):
        ...
        def assert_time_match(dt, st):
            """Compare a datetime object and an Elasticsearch string-formatted
            time as closely as possible.

            The strings are generated by a database function, and we
            can't use strftime to get the same level of sub-second
            precision, but we can check that it's basically the same
            time.

            :param dt: A datetime
            :param st: A string.
            """
            assert st.startswith(dt.strftime("%Y-%m-%dT%H:%M:%S."))
