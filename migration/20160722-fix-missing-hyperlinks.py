...
open_access_pools = _db.query(LicensePool).filter(LicensePool.open_access==True)

pools_with_open_access_links = _db.query(LicensePool).join(
        Hyperlink,
        Hyperlink.identifier_id==LicensePool.identifier_id
    ).filter(
...
open_access_pools_without_open_access_links = open_access_pools.filter(~LicensePool.id.in_(pool_ids_with_open_access_links))
